  ...
  return (
    <section id="portfolio" className="py-16 sm:py-20 bg-gradient-to-b from-white to-slate-50">
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-[95vw] w-full h-[85vh] sm:h-[90vh] p-0 gap-0 fixed top-[7.5vh] sm:top-[5vh] left-[2.5vw]">
          {selectedProject && (
            <div className="flex flex-col h-full bg-white rounded-lg overflow-hidden">
              {/* Modal Header - Sticky */}
              <div className="flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3 bg-[#0f172a] text-white border-b border-white/10 shrink-0">
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  <h3 className="text-sm sm:text-lg font-bold truncate">{selectedProject.name}</h3>
                  <span className="hidden sm:inline text-xs sm:text-sm text-gray-400">Demo Live</span>
                </div>
                <button
                  onClick={closeModal}
                  className="p-1.5 sm:p-2 hover:bg-white/10 rounded-lg transition-colors flex-shrink-0 min-h-10 min-w-10 flex items-center justify-center"
